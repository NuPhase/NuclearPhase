/proc/wash_mob(var/mob/living/washing)
	...
	if(washmask)
		if(mask)
			mask.clean_blood()
		for(var/descr in H.examine_descriptors)
			if(H.examine_descriptors[descr] == DESCRIPTOR_HAIR)
				H.remove_examine_descriptor(descr)
		H.add_examine_descriptor(SPAN_DESCRIPTION("[H.pronouns.His] hair looks freshly washed."), DESCRIPTOR_HAIR)
	if(washglasses)
	...
	for(var/descr in H.examine_descriptors)
		if(H.examine_descriptors[descr] == DESCRIPTOR_DIRTINESS || H.examine_descriptors[descr] == DESCRIPTOR_SMELL)
			H.remove_examine_descriptor(descr)
