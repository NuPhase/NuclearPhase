/mob/living/carbon/Initialize()
	...
	if(!bloodstr)
		bloodstr = new/datum/reagents/metabolism(1200, src, CHEM_INJECT)
	if(!reagents)
		reagents = bloodstr
	if(!touching)
		touching = new/datum/reagents/metabolism(5000, src, CHEM_TOUCH)
...
/mob/living/carbon/throw_item(atom/target, obj/item/item)
	...
	//actually throw it!
	src.visible_message("<span class='warning'>[message]</span>", range = min(itemsize*2,15))
