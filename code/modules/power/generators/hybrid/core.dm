...
/obj/machinery/power/hybrid_reactor/proc/meltdown_final()
	radio_announce("I believe it's too late... We're all gonna die.", ann_name)
	addtimer(CALLBACK(src, PROC_REF(stop_alarms)), 3 MINUTES)
	addtimer(CALLBACK(src, PROC_REF(produce_explosion)), 2 MINUTES)

/obj/machinery/power/hybrid_reactor/proc/meltdown_final_plasmaball()
	set waitfor = 0
	make_plasmaball()
