...
/obj/machinery/power/apc/on_update_icon()
	if (!status_overlays)
		...
		for(var/list/channel_leds in channel_overlays)
			channel_leds[POWERCHAN_OFF + 1] = emissive_overlay(icon,"apco[channel]",color=COLOR_RED)
			channel_leds[POWERCHAN_OFF_TEMP + 1] = emissive_overlay(icon,"apco[channel]",color=COLOR_ORANGE)
			channel_leds[POWERCHAN_OFF_AUTO + 1] = emissive_overlay(icon,"apco[channel]",color=COLOR_ORANGE)
			channel_leds[POWERCHAN_ON + 1] = emissive_overlay(icon,"apco[channel]",color=COLOR_LIME)
			channel_leds[POWERCHAN_ON_AUTO + 1] = emissive_overlay(icon,"apco[channel]",color=COLOR_BLUE)
			channel++
