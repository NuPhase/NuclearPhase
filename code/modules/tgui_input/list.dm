...
/proc/tgui_input_list(mob/user, message, title = "Select", list/items, default, timeout = 0)
	...
	if(user.get_preference_value(/datum/client_preference/tgui_input) != PREF_YES)
...
/datum/tgui_list_input/tgui_static_data(mob/user)
	var/list/data = list()
	data["swapped_buttons"] = user.get_preference_value(/datum/client_preference/tgui_input_swapped) == PREF_YES ? TRUE : FALSE
	data["large_buttons"] = user.get_preference_value(/datum/client_preference/tgui_input_large) == PREF_YES ? TRUE : FALSE
