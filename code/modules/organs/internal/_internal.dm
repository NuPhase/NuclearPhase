...
/obj/item/organ/internal/handle_germ_effects()
	. = ..()
	if(germ_level > INFECTION_LEVEL_TWO) //spread
		var/obj/item/organ/external/parent = GET_EXTERNAL_ORGAN(owner, parent_organ)
...
/obj/item/organ/internal/Process()
	...
	if(!owner)
		return
	if(owner.get_blood_perfusion() < 0.1)
		oxygen_starve(oxygen_deprivation_tick)
	else
		oxygen_deprivation = 100 - (owner.get_blood_perfusion()*100)
		oxygen_starve(oxygen_deprivation_tick * -1.5)
