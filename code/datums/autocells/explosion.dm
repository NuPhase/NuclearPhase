	...
	if(power >= get_config_value(/decl/config/num/iterative_explosives_z_threshold))
		...
		if((z_transfer & UP) && above_T)
			if(istype(above_T, /turf/simulated/open))
				...
				cell_explosion(above_T, power * get_config_value(/decl/config/num/iterative_explosives_z_multiplier), z_transfer=UP)
		if((z_transfer & DOWN) && below_T)
			if(istype(below_T, /turf/simulated/open))
				cell_explosion(below_T, power * 0.5, z_transfer=DOWN)
			else
				cell_explosion(below_T, power * get_config_value(/decl/config/num/iterative_explosives_z_multiplier), z_transfer=DOWN)
