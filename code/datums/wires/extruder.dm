...
/datum/wires/extruder/GetInteractWindow(mob/user)
...
/datum/wires/extruder/CanUse()
	var/obj/machinery/fabricator/A = holder
	if(A.panel_open)
		return 1
	return 0

/datum/wires/extruder/UpdateCut(index, mended)
	var/obj/machinery/fabricator/A = holder
	switch(index)
...
/datum/wires/extruder/proc/reset_flag(var/index, var/flag)
	var/obj/machinery/fabricator/A = holder
	if(A && !IsIndexCut(index) && flag)
		A.fab_status_flags &= ~flag
		Interact(usr)

/datum/wires/extruder/UpdatePulsed(index)
	if(IsIndexCut(index))
		return
