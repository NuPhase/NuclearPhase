...
/obj/structure/hygiene/sink/attackby(obj/item/O, var/mob/user)
	...
	if (istype(RG) && ATOM_IS_OPEN_CONTAINER(RG) && RG.reagents)
		RG.reagents.add_reagent(/decl/material/liquid/water/dirty2, min(RG.volume - RG.reagents.total_volume, RG.amount_per_transfer_from_this))
		user.visible_message(
		...
		if(REAGENTS_FREE_SPACE(O.reagents) >= 5)
			O.reagents.add_reagent(/decl/material/liquid/water/dirty2, 5)
...
/obj/structure/hygiene/faucet/proc/water_flow()
	...
	T.add_fluid(/decl/material/liquid/water/dirty2, min(75, fill_level - T.get_fluid_depth()))
